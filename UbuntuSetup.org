...
- i3wm
  - Install i3wm-themer
    ...
    Make sure `which python` returns `/usr/bin/python`, the installation script uses local python2.7
...
vim ./src/config.yaml # edit the path

./install_ubuntu.sh

python src/i3wm-themer.py --config config.yaml --load themes/008.json
    #+END_SRC
  - Display Management
    #+BEGIN_SRC shell
...
sudo apt-get install build-essential curl file git zsh p7zip-full
   #+END_SRC
